...
//		\Session::delete('login_hash');
        ...
        public function check_if_login_exists($outside_system_id, $outside_user_id)
        {
            $dublicate = \DB::select()->from('login')
                ->where_open()
                    ->where('outside_system_id','=', $outside_system_id)
                    ->and_where('outside_user_id', '=', $outside_user_id)
                ->where_close()
                ->execute(\Config::get('simpleauth.db_connection'));
            return count($dublicate);
        }
        
        public function create_login($outside_user_id, $outside_system_id, $user_id)
        {
            $exists = $this->check_if_login_exists($outside_system_id, $outside_user_id);
            if ($exists >0)
            {
                    return false;
            }
            
            \DB::insert('login')->columns(array(
                            'outside_system_id', 'outside_user_id', 'user_id'))
                        ->values(array(
                            $outside_system_id, $outside_user_id, $user_id))
                        ->execute(\Config::get('simpleauth.db_connection'));
            return true;
	 ...
	 * @return  bool
	 */
	public function create_user($outside_user_id, $outside_system_id, $email = null, $group = 1, Array $profile_fields = array(), $name, $surname)
	{
                if($email != null)
...
//		}
		...
		$result = \DB::insert(\Config::get('simpleauth.table_name'))
			...
			->execute(\Config::get('simpleauth.db_connection'));
                ...
                //if adding user was successful add in new entry into login table
                if ($result[0])
                {
                    $this->create_login($outside_user_id, $outside_system_id, $result[0]);
                }
	...
	public function create_login_hash()
	{
		...
		$login_hash = sha1(\Config::get('simpleauth.login_hash_salt').$this->user['id'].$last_login);
